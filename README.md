...
Interactive map of City of Girard, Ohio municipal water service accounts — **5,994 accounts across 5,568 service locations** — built as a civic data tool for exploring where the city's water system actually serves.

**Live site:** https://mikecostarella.github.io/GirardOhioWater/

## The civic finding

Girard's water system extends well beyond its city limits. A point-in-polygon analysis of every service location against Census TIGERweb boundaries shows:

| Jurisdiction | Locations |
|---|---|
| Girard | 3,960 |
| Liberty Township | 1,235 |
| Weathersfield Township | 369 |
| Youngstown | 3 |
| Warren | 1 |

**1,608 locations (~29%) — totaling 1,725 accounts — sit outside Girard city limits**, primarily in Liberty and Weathersfield Townships. Out-of-city locations are highlighted on the map with a magenta ring.
...
- **Street filter** — multi-select dropdown over 274 canonical street names, with an in-panel search box; combines with the jurisdiction filter (AND)
...
Open http://localhost:5173/GirardOhioWater/ — the base path matches the GitHub Pages deployment. Pushing to `main` triggers the GitHub Actions workflow, which builds and deploys to Pages (about two minutes). The site is a PWA, so hard-refresh (Ctrl+Shift+R) after a deploy to bypass the service worker cache.
...
- **`npm run assign-streets`** — joins each account to the source spreadsheet's `StreetName` column (by `AccountNumber`) and writes canonical street names onto accounts and locations. Looks for the spreadsheet in `../Data/` by default, or pass a path: `npm run assign-streets -- path\to\file.xlsx`.
...
Map tiles © [OpenStreetMap](https://www.openstreetmap.org/copyright) contributors. Boundary data from the U.S. Census Bureau TIGERweb services. Account data provided by the City of Girard.
